...
for z=1:size(cfg.SNR,2)
    ...
    % Receiver
        ...
        errors(k) = (abs(CFO*cfg.F_carrier-CFO_corr).^2)/(abs(CFO*cfg.F_carrier).^2);
    end
    MSE_CFO(z) = mean(errors);
...
figure;
semilogy(cfg.SNR,MSE_CFO,'bo-', 'LineWidth', 1.5);
grid on
xlabel('SNR (dB)');
ylabel('MSE (dB)');
xlim([cfg.SNR(1) cfg.SNR(end)]);
title('MSE - CFO estimation')

figure;
plot(cfg.SNR,MSE_CFO,'bo-', 'LineWidth', 1.5);
grid on
xlabel('SNR (dB)');
ylabel('MSE');
xlim([cfg.SNR(1) cfg.SNR(end)]);
title('MSE - absolute value - CFO estimation')
