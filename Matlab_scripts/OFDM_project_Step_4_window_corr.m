...
for i=1:size(cfg.SNR,2)
    ...
    signal{12} = blocks{13}(signal{11},h_temp,channel_mode);        % Equalizer - Zero Forcing - Channel temporal est.
...
for i=1:size(cfg.SNR,2)
    ...
    BER_no_pilot(i) = mean(signal{1}(size_pream+1:end)~=signal{end});
...
for i=1:size(cfg.SNR,2)
    ...
    signal{10} = blocks{10}(signal{9},channel);                     % FFT
    [h_temp,h_freq] = blocks{11}(signal{2},signal{10});             % channel estimation - preamble
    signal{12} = blocks{13}(signal{10},channel,channel_mode);        % Equalizer - Zero Forcing - Channel temporal est.
    signal{13} = blocks{14}(signal{12});                            % Demapping
...
hold on
semilogy(cfg.SNR,BER_no_pilot,'rx-', 'LineWidth', 1.5);
semilogy(cfg.SNR,BER_pilot,'m+-', 'LineWidth', 1.5);
grid on
xlabel('SNR (dB)');
ylabel('BER');
xlim([cfg.SNR(1) 15]);
legend('BER-true', 'BER-no-pilot', 'BER-pilot');
