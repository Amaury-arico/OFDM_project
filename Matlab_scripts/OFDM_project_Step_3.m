...
for i=1:size(cfg.SNR,2)
    ...
    BER_time_error(i) = mean(signal{1}(size_pream+1:end)~=signal{end});
    % Normalized MSE calculation - Time impulse comparison
    NMSE_STO(i) = sum(abs(signal{8}-signal{9}).^2)/(abs(signal{8}).^2);
...
for i=1:size(cfg.SNR,2)
    ...
    BER_no_error(i) = mean(signal{1}(size_pream+1:end)~=signal{end});
...
figure;
semilogy(cfg.SNR,BER_no_error,'bo-', 'LineWidth', 1.5);
hold on
semilogy(cfg.SNR,BER_time_error,'rx-', 'LineWidth', 1.5);
grid on
xlabel('SNR (dB)');
ylabel('BER');
xlim([cfg.SNR(1) 15]);
legend('BER-no-error', 'BER-time-error','BER-CFO-error','BER-time-CFO-error');
title('Bit Error Rate evolution')

figure;
semilogy(cfg.SNR,NMSE_STO,'bo-', 'LineWidth', 1.5);
grid on
xlabel('SNR (dB)');
ylabel('NMSE (%)');
xlim([cfg.SNR(1) cfg.SNR(end)]);
title('Normalized MSE - STO estimation')
